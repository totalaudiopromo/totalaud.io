...
class AudioEngine {
  ...
  private playSynth(config: SoundConfig, startTime: number) {
    ...
    gain.gain.linearRampToValueAtTime(this.volume, startTime + envelope.attack)
    ...
    gain.gain.setValueAtTime(this.volume * envelope.sustain, releaseStart)
...
export const agentSounds = {
  ...
  insight: {
    start: () => {
      ...
      audioEngine.playChord([261.63, 329.63, 392.0, 523.25], 600) // C4-E4-G4-C5 harmonic series
    },
    complete: () => {
      // Harmonic sweep upward
      const frequencies = [261.63, 293.66, 329.63, 349.23, 392.0] // C-D-E-F-G ascending
