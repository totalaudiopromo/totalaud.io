...
/**
 * Export adaptive rewrites as JSON
 * Phase 21 - Export rewrite history
 */
export function exportAdaptiveRewritesJSON(score: AdaptiveCreativeScore): string {
  return JSON.stringify(score.rewriteHistory, null, 2)
}

/**
 * Export adaptive summary as Markdown
 * Phase 21 - Human-readable summary of adaptive changes
 */
export function exportAdaptiveSummaryMarkdown(score: AdaptiveCreativeScore): string {
  const lines: string[] = []

  lines.push('# Adaptive Performance Summary')
  lines.push('')
  ...
  // Adaptive Metadata
  lines.push('## Adaptive Metadata')
  lines.push('')
  lines.push(`- **Total Rewrites:** ${score.adaptiveMetadata.totalRewrites}`)
  lines.push(
    `- **Last Rewrite:** ${new Date(score.adaptiveMetadata.lastRewriteTimestamp).toLocaleString('en-GB')}`
  )
  lines.push(
    `- **Duration Drift:** ${score.adaptiveMetadata.durationDrift > 0 ? '+' : ''}${score.adaptiveMetadata.durationDrift.toFixed(1)}%`
  )
  lines.push(
    `- **Structure Preserved:** ${score.adaptiveMetadata.structurePreserved ? 'Yes' : 'No'}`
  )
  lines.push('')

  // Rewrite History
  lines.push('## Rewrite History')
  lines.push('')

  if (score.rewriteHistory.length === 0) {
    lines.push('_No rewrites applied_')
  } else {
    // Group by scene
    const byScene: Record<number, typeof score.rewriteHistory> = {}
    score.rewriteHistory.forEach((rewrite) => {
      if (!byScene[rewrite.scene]) {
        byScene[rewrite.scene] = []
      }
      byScene[rewrite.scene].push(rewrite)
    })

    Object.entries(byScene).forEach(([sceneIndex, rewrites]) => {
      lines.push(`### Scene ${parseInt(sceneIndex) + 1} (${rewrites.length} rewrites)`)
      lines.push('')

      rewrites.forEach((rewrite) => {
        lines.push(`**Bar ${rewrite.bar}** - ${rewrite.changeDescription}`)
        lines.push(`- Property: \`${rewrite.affectedProperty}\``)
        if (rewrite.oldValue !== null && rewrite.newValue !== null) {
          const oldVal =
            typeof rewrite.oldValue === 'number'
              ? rewrite.oldValue.toFixed(2)
              : rewrite.oldValue
          const newVal =
            typeof rewrite.newValue === 'number'
              ? rewrite.newValue.toFixed(2)
              : rewrite.newValue
          lines.push(`- Change: ${oldVal} → ${newVal}`)
        }
        lines.push(`- Directive: [${rewrite.directive.type}] ${rewrite.directive.reasoning}`)
        lines.push('')
      })
    })
  }
  ...
  lines.push('---')
  lines.push('')
  lines.push('_Generated by totalaud.io Adaptive Performance Engine (Phase 21)_')
  lines.push('')

  return lines.join('\n')
}

// Import AdaptiveCreativeScore type
import type { AdaptiveCreativeScore } from './creativeScoreRewriter'
