      ...
      {patterns.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {patterns.map((item, i) => (
            <motion.div
              key={item.id}
              className="relative group p-5 bg-[#161A1D] rounded-xl border border-white/5 overflow-hidden transition-colors hover:border-ta-cyan/30"
              ...
              <div className="relative z-10 space-y-3">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-start gap-3">
                    <div className="mt-0.5 p-1.5 rounded-lg bg-ta-cyan/10 text-ta-cyan">
                      <LightBulbIcon className="w-4 h-4" />
                    </div>
                    <p className="text-sm font-medium text-ta-white leading-snug">{item.pattern}</p>
                  </div>
                ...
                <div className="flex items-center gap-2 pt-2 border-t border-white/5">
                  <span className="text-xs text-ta-grey">Estimated Impact:</span>
                  <span className="px-2 py-0.5 rounded-full text-[10px] uppercase tracking-wider font-medium bg-white/5 text-ta-white/80 border border-white/10 group-hover:border-ta-cyan/20 group-hover:text-ta-cyan transition-colors">
                    {item.impact}
                  </span>
                </div>
              </div>
