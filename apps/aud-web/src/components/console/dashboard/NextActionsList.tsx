...
export function NextActionsList({ actions: initialActions }: NextActionsListProps) {
  ...
  const priorityColor = (priority: string) => {
    ...
    return 'bg-tap-cyan shadow-tap-cyan/20'
  }
    ...
    <Card className="h-full bg-[#161A1D] border border-white/5 flex flex-col">
      <div className="flex items-center justify-between mb-6 px-1">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-lg font-medium text-tap-white tracking-tight leading-none">
              Next Actions
            </h3>
            <p className="text-xs text-tap-grey mt-1">Focus on these tasks to improve coverage</p>
          </div>
        </div>
          ...
          {visibleActions.length === 0 ? (
            ...
            >
              <div className="w-12 h-12 rounded-full bg-white/5 flex items-center justify-center mb-4">
                <CheckCircleIcon className="w-6 h-6 text-tap-grey/50" />
              </div>
              <p className="text-sm text-tap-white font-medium">All caught up!</p>
              <p className="text-xs text-tap-grey mt-1">No pending actions for your campaigns.</p>
            </motion.div>
          ) : (
            visibleActions.map((item, i) => (
              ...
              >
                {/* Custom Checkbox */}
                <div className="mt-1 flex-shrink-0 relative w-5 h-5 rounded-full border border-tap-grey/30 group-hover:border-tap-cyan transition-colors overflow-hidden bg-[#0A0B0C]">
                  <div className="absolute inset-0 bg-tap-cyan opacity-0 group-hover:opacity-10 transition-opacity" />
                  <CheckCircleIcon className="w-full h-full text-tap-black opacity-0 group-hover:opacity-100 scale-0 group-hover:scale-100 transition-all duration-300" />
                </div>

                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-4 mb-1.5">
                    <p className="text-sm font-medium text-tap-white leading-snug group-hover:text-white transition-colors">
                      {item.action}
                    </p>
                  ...
                  <div className="flex items-center gap-2">
                    <span className="px-1.5 py-0.5 rounded text-[10px] uppercase tracking-wider font-medium bg-white/5 text-tap-grey group-hover:bg-white/10 transition-colors border border-white/5">
                      {item.category}
                    </span>
                  </div>
                </div>
