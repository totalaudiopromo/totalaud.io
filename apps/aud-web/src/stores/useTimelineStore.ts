      ...
      loadFromSupabase: async () => {
        ...
        try {
          ...
          const { data, error } = await supabase
            .from('user_timeline_events')
            .select(
              'id, user_id, lane, title, event_date, colour, description, url, tags, source, opportunity_id, tracker_campaign_id, tracker_synced_at, contact_id, created_at, updated_at'
            )
            .eq('user_id', user.id)
